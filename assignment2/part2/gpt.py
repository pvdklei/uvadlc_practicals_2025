...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(
        ...
        pos_sin = torch.sin(angles)  # Shape: (T, dim // 2)
        pos_cos = torch.cos(angles)  # Shape: (T, dim // 2)
        ...
        xq_rot[..., 0::2] = xq_real * pos_cos - xq_imag * pos_sin
        xq_rot[..., 1::2] = xq_real * pos_sin + xq_imag * pos_cos
        xk_rot[..., 0::2] = xk_real * pos_cos - xk_imag * pos_sin
        xk_rot[..., 1::2] = xk_real * pos_sin + xk_imag * pos_cos
    ...
    def forward(self, x):
        ...
        q, k, v = self.c_attn(x).split(C, dim=2)  # each of shape (B, T, C)
        ...
        if self.use_flash_attn:
            ...
            att = att / math.sqrt(k.size(-1))  # Scale the attention scores
            # Apply causal mask
            att = att.masked_fill(
